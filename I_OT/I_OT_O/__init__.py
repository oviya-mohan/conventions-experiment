...
############################ ONLY FOR FIRST APP ################################
        ...
        #first_total defined in settings.py t0 access from second app
...
def custom_export(players):
    ...
    yield['participant_number', 'participant_code', 'answer', 'age', 'sex', 'name', 'email']
    for p in players:
        answer = p.Q
        name = p.name
        age = p.age
        ...
        if p.round_number == 1 or p.round_number == Constants.num_rounds:
            yield[p.id_in_group, p.participant.code, answer, age, sex, name, email]
